 ...
 * FileAPI Ч a set of  javascript tools for working with files. Multiupload, drag'n'drop and chunked file upload. Images: crop, resize and auto orientation by EXIF.
 */
			...
			upload: function (options){
				...
				_nextFile = function (){
					...
					if( _file && _file.name === api.expando ){
						_file = null;
						api.log('[warn] FileAPI.upload() Ч called without files');
					}
...
(function (api, document, undef) {
	...
	Image.transform = function (file, transform, autoOrientation, fn){
		...
		// @todo: ќло-ло, нужно рефакторить это место
		if( file.width ){
				...
				patch: function (){
					...
					_inherit(api, {
						getFiles: function (input, filter, callback){
							...
							if( !files ){
								// ‘айлов нету, вызываем родительский метод
